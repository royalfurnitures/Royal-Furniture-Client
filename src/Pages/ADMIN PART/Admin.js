...
export const Admin = () => {
  ...
  const handleLogoutmodel = (action)=>{
    if(action === true){
      HandleLogout();
    }
  ...
  return (
    <Fragment>
      {
        isLogoutPopup ?
        <Popup handleModel={handleLogoutmodel} isLogout={true} />
        :
        null
      }
    <div className='bg-white'>
      <div className='flex items-center fixed left-0 right-0 top-0 text-black z-50 text-[20px] lg:text-[20px]' > 
        ...
        <div className='text-lg lg:text-2xl font-bold text-orange-500 flex items-center justify-start py-10 h-[60px] shadow-sm fixed left-0 right-0 top-0 border px-14 bg-white '> <img src={Logo} alt="logo" className='h-[40px]' /> </div>
         ...
         <div className={` ${ismenu ? "block" :"hidden"} shadow-sm h-[93vh] lg:h-[100vh]  lg:block  lg:w-[300px]  border-[2px]  px-4 fixed z-10 top-[60px] lg:top-[80px] bg-white `}>
          <div className='relative mb-10'>
            ...
            <div className='flex gap-[10px]  flex-col px-4'>
               ...
               <Link to="/admindesignideas"><div className={`${currenttab === "admindesignideas" ? "bg-orange-400 rounded-lg p-2 text-white cursor-default text-bold":"p-2 cursor-pointer text-bold"} flex gap-3 items-center`} onClick={()=>{setTabIndex(8)}}><FaBloggerB />Design Ideas</div></Link>
               <div className={`${currenttab === "logout" ? "bg-orange-400 rounded-lg p-2 text-white cursor-default text-bold":"p-2 cursor-pointer text-bold"} flex gap-3 items-center`} onClick={()=>{setIsLogoutPopup(true) }}><IoLogOutOutline /> Logout</div>                 
            </div>
         </div>
    </div>
    </Fragment>
